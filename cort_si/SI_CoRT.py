...
def SI_parametric(n_target, p, K, target_data, source_data, lamda_big, lamda_small, T, s_len):
    ...
    model = Lasso(alpha=lamda_big, fit_intercept=False, tol=1e-10, max_iter=10000000)
    ...
    z_min = -20  * tn_sigma
    z_max = 20 * tn_sigma
    z_interval = parametric.solve_truncation_CoRT(z_min, z_max, X_target, folds, source_data, a_global, b_global, lamda_small, lamda_big, p, K, T, M_obs)
...
def SI_over_conditioning(n_target, p, K, target_data, source_data, lamda_big, lamda_small, T, s_len):
    CoRT_model = CoRT_builder.CoRT(alpha=lamda_small)
    similar_source_index = CoRT_model.find_similar_source(n_target, K, target_data, source_data, T=T, verbose=False)
    X_combined, y_combined = CoRT_model.prepare_CoRT_data(similar_source_index, source_data, target_data)

    model = Lasso(alpha=lamda_big, fit_intercept=False, random_state=42)
    model.fit(X_combined, y_combined.ravel())
    beta_hat_target = model.coef_[-p:]
    ...
    L_base_agu, R_base_agu = over_conditioning.get_Z_base_aug(etajTy, folds, source_data, a_global, b_global, lamda_small, K, T)
    L_val, R_val = over_conditioning.get_Z_val(folds, T, K, a_global, b_global, etajTy, lamda_small, source_data)
    L_CoRT, R_CoRT, Az = over_conditioning.get_Z_CoRT(X_combined, similar_source_index, lamda_big, a_global, b_global, source_data, etajTy)
