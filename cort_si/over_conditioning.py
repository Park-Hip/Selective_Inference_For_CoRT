...
def get_Z_base_aug(z_obs, folds, source_data, a_global, b_global, lamda, K, T):
  list_R = []
  L_final = - np.inf
  R_final = np.inf
  ...
  for t in range(T):
    ...
    L_base, R_base = solve_interval.compute_lasso_interval(X_base, a_base, b_base, lamda, z_obs)
    ...
    for k in range(K):
      ...
      L_aug, R_aug = solve_interval.compute_lasso_interval(X_aug, a_aug, b_aug, lamda, z_obs)
...
def get_Z_val(folds, T, K, a_global, b_global, z_obs, alpha_val, source_data):
  ...
  for t in range(T):
    ...
    u_base, v_base = utils.get_u_v(X_base_train, a_base_train, b_base_train, z_obs, alpha_val)
    ...
    for k in range(K):
      ...
      u_aug, v_aug = utils.get_u_v(X_aug_train, a_aug_train, b_aug_train, z_obs, alpha_val)
...
def get_Z_CoRT(X_combined, similar_source_index, alpha_val, a_global, b_global, source_data, z_obs):
  ...
  clf = Lasso(alpha=alpha_val, fit_intercept=False, tol=1e-10, max_iter=10000000)
