...
def solve_truncation_interval(z_min, z_max, folds, source_data, a_global, b_global, lamda, K, T): 
    interval = {}
    for k in range(K):
        for t in range(T):
            fold_indices = []
            ...
            interval[(k, t)] = []
            while z1 < z1_max:
                L_base, R_base = solve_interval.compute_lasso_interval(X_base, a_base, b_base, lamda, z1)
                ...
                while z2 < z2_max:  
                    L_aug, R_aug = solve_interval.compute_lasso_interval(X_aug, a_aug, b_aug, lamda, z2)
                    z3 = z2
                    z3_max = min(R_aug, z2_max)
                    while z3 < z3_max:
                        u_base, v_base = utils.get_u_v(X_base, a_base, b_base, z3, lamda)
                        X_val = folds[t]["X"]
                        ...
                        u_aug, v_aug = utils.get_u_v(X_aug, a_aug, b_aug, z3, lamda)
    ...
    return interval
def solve_truncation_CoRT(z_min, z_max, X_target, folds, source_data, a_global, b_global, lamda_small, lamda_big, p, K, T, M_obs):
    interval = solve_truncation_interval(z_min, z_max, folds, source_data, a_global, b_global, lamda_small, K, T)
    ...
    while z_k < z_max:
        ...
        L_CoRT, R_CoRT, Az = solve_interval.compute_lasso_interval(X_combined_new, a_global, b_global, lamda_big, z_k, similar_source_index, source_data)
