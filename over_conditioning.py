...
def get_Z_CoRT(p_original, X_combined, similar_source_index, a_global, b_global, source_data, z_obs):
  ...
  threshold = 1e-10
  active_indices = [idx for idx, coef in enumerate(clf.coef_) if abs(coef) > threshold]
  inactive_indices = [idx for idx, coef in enumerate(clf.coef_) if abs(coef) <= threshold]
  ...
  if m == 0:
    p = (1 / lamda) * X_combined.T @ a_CoRT
    q = (1 / lamda) * X_combined.T @ b_CoRT
    ...
    u = np.linalg.pinv(X_M.T @ X_M) @ (X_M.T @ a_CoRT - lamda * s_M)
    v = np.linalg.pinv(X_M.T @ X_M) @ (X_M.T @ b_CoRT)
    p = (1 / lamda) * X_Mc.T @ (np.eye(n) - P_M) @ a_CoRT + X_Mc.T @ X_M @ np.linalg.pinv(X_M.T @ X_M) @ s_M
    q = (1 / lamda) * X_Mc.T @ (np.eye(n) - P_M) @ b_CoRT
