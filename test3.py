...
def get_Z_val(z_obs, folds, T, K, a_global, b_global, alpha_val, source_data):
    ...
    for k in range(K):
      for t in range(T):
        ...
        delta = A_dif * z_obs * z_obs + B_dif * z_obs + C_dif
        if delta < 0:
...
def update_Z_val(val, z_obs, folds, T, K, a_global, b_global, alpha_val, source_data, similar_source, cnt_vote, is_similar):
    L_final = - np.inf
    R_final = np.inf
    ...
    return L_vote, R_vote, similar_source, cnt_vote, is_similar, oke
...
def pivot(A, list_active_set, list_zk, etaj, etajTy, tn_mu, cov):
    ...
    try:
        ...
        for i in range(len(list_active_set)):
            ...
            if np.array_equal(A, np.sort(list_active_set[i])):
                 z_interval.append([list_zk[i], list_zk[i+1] - 1e-10]) # Removed 1e-10 to keep precision
